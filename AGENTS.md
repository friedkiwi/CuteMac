...
- While interactive host input is held, temporarily pace unlimited execution at realtime so a normal wall-clock click or keypress does not span thousands of guest VBLs and trigger Finder auto-repeat. Queue the release before removing this temporary throttle; do not change the profile's configured speed.
...
- Mac Plus audio scans the selected 370-byte sound page at 22.255 kHz, honors VIA sound enable and volume, and publishes frontend-neutral signed 16-bit PCM. `CuteMacSession` consumes the same reusable machine audio contract through Qt Multimedia; debug capture/export remains separately opt-in.
