...
- Network backends are selected from the `config::networkBackendDescriptors()` registry, which drives TOML naming, profile validity, and the configuration UI from one table. Profile validity is gated on compile-time support, never on runtime availability: a profile written where Npcap is installed must still load and save where it is not, with `networkBackendAvailability()` explaining why the link is idle.
- `PacketNetworkBackend::poll()` runs on the emulation thread once per scheduler quantum. No backend method may block; a backend needing blocking I/O owns a thread and hands frames over through a queue.
...
- Optional dependencies must report their state at configure time and fail loudly when a declared manifest dependency is missing. Never let a shipped feature degrade silently: `CUTEMAC_ENABLE_SLIRP` and `CUTEMAC_ENABLE_PCAP` are on by default, forced off only for wasm, and their `pkg_check_modules`/`find_library` calls are `REQUIRED`.
- Bridged networking builds against vcpkg `libpcap` everywhere except Windows, where the vcpkg port is configured `-DPCAP_TYPE=null` and enumerates no interfaces. Windows instead uses the `npcap-sdk` overlay port in `cmake/vcpkg-ports`, declared through `vcpkg-configuration.json`; overlay ports live in the repository, never in the pinned `third_party/vcpkg` submodule.
- Npcap is not open source and may not be redistributed without a licence from the Nmap Project. Ship no part of its runtime: the overlay port installs headers and import libraries only, `wpcap.dll` is delay-loaded from the user's own installation (System32, then System32\\Npcap), and the Windows packaging step fails if `wpcap.dll`, `Packet.dll` or `npcap.dll` reaches an artifact.
- Delay-load directives belong on `CuteMacCore`'s `INTERFACE` link options. It is a static library, so a `PRIVATE` `/DELAYLOAD` would silently do nothing and a load-time import would stop `CuteMac.exe` starting on any machine without Npcap.
