...
- `EmulatorWindow` uses an integrated absolute pointer on every platform. Map widget coordinates directly to guest coordinates, hide the host cursor only over the emulated display, and never call `grabMouse()` or `QCursor::setPos()`. Leaving the emulated display, losing focus, or opening a modal session dialog must restore the host cursor and release any held guest mouse button.
...
- Keep host key mapping, capture policy, and framebuffer conversion in session frontend adapters. Wayland/wasm remain on the no-warp absolute-pointer path. Keep configuration device tables out of inline edit mode; Wayland input-method focus recursion can otherwise overflow the stack.
