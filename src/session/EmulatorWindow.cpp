...
public:
        ...
        , m_image(512, 342, QImage::Format_RGB32)
    {
        ...
        m_hostRelativeMouseCapture = createHostRelativeMouseCapture();
    ...
    void releaseMouseCapture()
    {
        ...
        if (m_relativeCapture) {
            ...
            if (m_nativeRelativeCapture) {
                m_nativeRelativeCapture = false;
                if (m_hostRelativeMouseCapture) m_hostRelativeMouseCapture->stop();
            } else if (mouseGrabber() == this) {
                releaseMouse();
            }
        }
...
private:
    ...
    void sendMouseEvent(QMouseEvent* event)
    {
        ...
        if (m_mouseCaptured && m_relativeCapture) {
            ...
            if (!delta.isNull()) {
                sendRelativeMouseDelta(delta.x(), delta.y());
    ...
    bool m_nativeRelativeCapture = false;
    ...
    QImage m_image;
    std::unique_ptr<HostRelativeMouseCapture> m_hostRelativeMouseCapture;
