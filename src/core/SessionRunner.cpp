...
void SessionRunner::setInteractiveInputActive(bool active)
{
    m_interactiveInputActive = active;
#if !defined(Q_OS_WASM)
    m_wake.notify_all();
#endif
}
...
void SessionRunner::runHostFrame()
{
    ...
    if (m_running && !m_paused) {
        ...
        if (m_speed == config::RuntimeSpeed::Unlimited && !m_interactiveInputActive
            && !m_audioPlaybackActive && !m_machineAudioPlaybackActive) {
...
void SessionRunner::workerLoop()
{
    ...
    auto previousRealtimeThrottle = m_speed.load() == config::RuntimeSpeed::Realtime;
    while (m_running) {
        if (m_paused) {
        ...
        const auto realtimeThrottle = currentSpeed == config::RuntimeSpeed::Realtime
            ...
            || m_machineAudioPlaybackActive.load();
