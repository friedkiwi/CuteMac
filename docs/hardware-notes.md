...
- The card forwards the DP8390's own receive filter to the backend: PAR0-PAR5 writes (page 1, `csWrite` cases `0x41`-`0x46`) publish the station address, and RCR bit 4 publishes promiscuous mode. The card still filters in software; the backend uses the same values to install a host-side BPF filter (`ether dst <mac> or ether broadcast or ether multicast`) so a busy segment is not copied into the emulator frame by frame. A guest driver that reprograms PAR after reset must therefore reach the backend, which is why the notification hangs off register writes rather than construction.
- Bridged capture opens with immediate mode and non-blocking reads, and rejects any interface whose `pcap_datalink()` is not `DLT_EN10MB` — on macOS, `utun*` and similar report `DLT_NULL` and would otherwise present tunnel payloads as Ethernet frames. `pcap_setdirection(PCAP_D_IN)` keeps the card's own transmissions from returning as receives on Linux and BSD; Windows rejects the call and Npcap does not loop sent frames back anyway.
